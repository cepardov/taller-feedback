...
<table>
	...
	<% for(Marca cli : listadoMarca){ %>
		<tr>
			...
			<td><a href="<%= request.getContextPath() %>/marca/ingresomarca.jsp?id=<%= cli.getId()%>">
			editar</a></td>
                        <td><a onclick="return confirm('Esta seguro?');" href="<%= request.getContextPath() %>/marca/eliminarmarca.jsp?id=<%= cli.getId()%>">
			eliminar</a></td>
		</tr>
	<%} %>
</table>
