...
<table>
	...
	<% for(Repuesto rep :listadoRepuesto){ %>
		<tr>
			...
			<td><a href="<%= request.getContextPath() %>/repuesto/ingresorepuesto.jsp?id=<%= rep.getIdrepuesto()%>">
			editar</a></td>
                        <td><a onclick="return confirm('Esta seguro?');" href="<%= request.getContextPath() %>/repuesto/eliminarepuesto.jsp?id=<%= rep.getIdrepuesto()%>">
			eliminar</a></td>
		</tr>
	<%} %>
</table>
